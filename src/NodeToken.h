...
void displayStat(char *text) {
  pushToConsole(string_format(" %s :max used memory: %ld maxstack:%ld  started "
                              "%d free mem:%ld consumed %ld time:%dms",
                              text, __maxMemUsage, __MaxStackMemory, __startmem,
...
void updateMem() {
  ...
  // __MaxStackMemory, esp_get_free_heap_size());
#endif
}
...
public:
  ...
  NodeToken(NodeToken nd, nodeType tt) {
    ...
    if (tt == defLocalVariableNode) {
      ...
      if (nd.getVarType()->_varType == __uint32_t__ ||
          nd.getVarType()->_varType == __float__ ||
          nd.getVarType()->_varType == __CRGB__ ||
          nd.getVarType()->_varType == __int__ ||
          nd.getVarType()->_varType ==
              __userDefined__) //|| nd.getVarType()->_varType == __CRGBW__)
      {
        if (stack_size % 4 != 0) {
          if (nd.getVarType()->_varSize % 2 == 0)
            ...
            delta = nd.getVarType()->_varSize - stack_size % 4 + 1;
                 ...
                 nd.getVarType()->_varType == __s_int__) {
        if (stack_size % 2 != 0) {
          delta = 1;
        }
      }
  ...
  void copyChildren(NodeToken *k) {
    if (k->children_size() > 0) {
      for (NodeToken *lm : *k->children) {
        NodeToken *hj = addChild(*lm);
        if (lm->children_size() > 0) {
          for (NodeToken *kl : *lm->children) {
            hj->addChild(*kl);
          }
        }
      }
  ...
  void replaceTargetText(string t) { all_targets.replaceText(textref, t); }
  NodeToken *getChildAtPos(int pos) {
    if (children == NULL)
      return NULL;
    if (pos >= 0 and pos < children_size())
      return (*children)[pos];
    else
      return NULL;
  }
  char *getTokenText() { return all_text.getText(textref); }
  void setTokenText(string t) { textref = all_text.addText(t); }
  void addTargetText(string t) { target = all_targets.addText(t); }
  char *getTargetText() { return all_targets.getText(target); }
  void setTargetText(string t) { target = all_targets.addText(t); }
      ...
      if (target != EOF_TEXTARRAY) {
        ...
        if (i > -1) {
          ...
          printf("member %s not foudn in %s\n", getTargetText(),
                 _userDefinedTypes[_vartype].varName.c_str());
          return NULL;
        }
      }
...
public:
  ...
  bool findCandidate(char *str) {
    ...
    for (vector<NodeToken *>::iterator it = _functions.begin();
  ...
  void findVariable(char *t, bool isCreation) {
    ...
    if (variables != NULL) {
      for (vector<NodeToken *>::iterator it = variables->begin();
           it != variables->end(); ++it) {
    ...
    if (!isCreation) {
      ...
      while (c_cntx != NULL) {
        ...
        if (c_cntx->variables != NULL) {
          for (vector<NodeToken *>::iterator it = c_cntx->variables->begin();
               it != c_cntx->variables->end(); ++it) {
...

{
  ...
  // ,nd->_token->text.c_str()); NodeToken var = NodeToken(_var);
...

{
  ...
  if (__nd->children_size() > 0) {
    for (vector<NodeToken *>::iterator it = __nd->children->begin();
...

{
  ...
  case __float__:
    ...
    bufferText->addAfterNoDouble(
        string_format("float.s f%d,a%d,0", regnum, regnum));
  ...
  case __int__:
    ...
    case __float__:
      //  bufferText->sp.pop();
      bufferText->addAfterNoDouble(
          string_format("trunc.s a%d,f%d,0", regnum, regnum));
      // bufferText->sp.push(bufferText->get());
      break;
    default:
      break;
...
void _visitstoreLocalVariableNodeAsRegister(NodeToken *nd) {
  bufferText->addAfter(
      bufferText->sp.pop(),
      string_format("mov a%d,a%d", nd->target, register_numl.get()));
}
void _visitlocalVariableNodeAsRegister(NodeToken *nd) {
  bufferText->addAfter(
      string_format("movr a%d,a%d", register_numl.get(), nd->target));
  bufferText->sp.push(bufferText->get());
...
void _visitnumberNode(NodeToken *nd) {
  ...
  if (nd->children_size() > 0) {
    ...
    if (nd->getVarType()->_varType == __float__) {
      ...
      // point_regnum++;
      bufferText->addAfter(
          string_format("l32r a%d,@_%s_%d", 8, "local_var", local_var_num));
      bufferText->addAfter(
          string_format("lsi f%d,a%d,0", register_numl.get(), 8));
      bufferText->sp.push(bufferText->get());
      // point_regnum--;
      local_var_num++;
      register_numl.decrease();
      ...
      if (__num >= 2048) {
        ...
        bufferText->addAfter(
            string_format("l32r a%d,@_%s_%d", 8, "local_var", local_var_num));
        bufferText->addAfter(
            string_format("l32i a%d,a%d,0", register_numl.get(), 8));
        bufferText->sp.push(bufferText->get());
        // point_regnum--;
        local_var_num++;
        register_numl.decrease();
        ...
        bufferText->addAfter(
            string_format("movi a%d,%d", register_numl.get(),
                          __num)); // nd->_token->text.c_str()));
        bufferText->sp.push(bufferText->get());
        register_numl.decrease();
      }
    } else {
      ...
      if (__num >= 2048 or __num <= -2047) {
        ...
        bufferText->addAfter(string_format(
            "movi a%d,%d", register_numl.get(),
            __num)); // nd->_token->text.c_str()));
                     // printf("on a %s\n",bufferText->current().c_str());
        bufferText->sp.push(bufferText->get());
        register_numl.decrease();
...
void _visitunitaryOpNode(NodeToken *nd) {
  ...
  if (nd->type == TokenNot) {
    ...
    bufferText->addAfter(string_format("bnez a%d,label_not_%d",
                                       register_numl.get(), for_if_num));
...
void _visitoperatorNode(NodeToken *nd) {
  ...
  case TokenAddition: {
    ...
    string new_line = string_format(
        "%s %s%d,%s%d,%s%d", asmInstructionsName[asmInstr].c_str(),
        getRegType(asmInstr, 0).c_str(), register_numl.get(),
        getRegType(asmInstr, 1).c_str(), register_numl.get(),
        getRegType(asmInstr, 2).c_str(), register_numr.get());

    string _last = bufferText->current();
    ; // bufferText->back();
    ...
    if (_last.compare(0, tocmp.size(), tocmp) == 0) {
      ...
      if (b >= -128 and b <= 127) {
        ...
        bufferText->addAfter(string_format(
            "addi a%d,a%d,%d", register_numl.get(), register_numl.get(), b));
      } else {
  ...
  case TokenShiftLeft:
    ...
    // bufferText->addAfter(string_format("sub
    // a%d,a8,a%d",register_numr.get(),register_numr.get()).c_str());
    bufferText->addAfter(string_format("ssl a%d", register_numr.get()).c_str());
    bufferText->addAfter(
        string_format("sll a%d,a%d", register_numl.get(), register_numl.get())
            .c_str());
    break;
  case TokenShiftRight:
    ...
    // a%d,a8,a%d",register_numr.get(),register_numr.get()).c_str());
    bufferText->addAfter(
        string_format("wsr a%d,3", register_numr.get()).c_str());
    bufferText->addAfter(
        string_format("srl a%d,a%d", register_numl.get(), register_numl.get())
            .c_str());
    break;
  case TokenSubstraction: {
    ...
    // bufferText->addAfter(string_format("sub a%d,a%d,a%d",
    // register_numl.get(), register_numl.get(), register_numr.get())); return;
  } break;
  case TokenSlash:
    // bufferText->addAfter(string_format("quou a%d,a%d,a%d",
    // register_numl.get(), register_numl.get(), register_numr.get()));
    if (ff) {
  ...
  case TokenStar:
    ...
    bufferText->addAfter(string_format(
        ...
        getRegType(asmInstr, 1).c_str(), register_numl.get(),
        getRegType(asmInstr, 2).c_str(), register_numr.get()));
    // bufferText->addAfter(string_format("mull a%d,a%d,a%d",
    // register_numl.get(), register_numl.get(), register_numr.get())); return;
    break;
  case TokenPlusPlus:
    if (nd->getChildAtPos(0)->isPointer &&
        nd->getChildAtPos(0)->children_size() == 0) {
      bufferText->addAfter(string_format(
          "addi a%d,a%d,%d", register_numl.get(), register_numl.get(),
          nd->getChildAtPos(0)->getVarType()->total_size));
    } else {
      bufferText->addAfter(string_format("addi a%d,a%d,1", register_numl.get(),
                                         register_numl.get()));
    }
    // return;
    break;
  case TokenMinusMinus:
    ...
    break;
  case TokenModulo:
    bufferText->addAfter(string_format("remu a%d,a%d,a%d", register_numl.get(),
                                       register_numl.get(),
                                       register_numr.get()));
    //  return;
    break;
    ...
    if (nd->getChildAtPos(1)->_nodetype == numberNode) {
      ...
      if (__num > 2) {
        if (ff) {
          bufferText->blankCurrent();
          bufferText->addAfter(
              string_format("mov.s f10,f%d", register_numl.get()));
          asmInstr = muls;
        } else {
          bufferText->addAfter(
              string_format("mov a10,a%d", register_numl.get()));
          asmInstr = mull;
        }
        for (int k = 1; k < __num; k++) {
          bufferText->addAfter(string_format(
              "%s %s%d,%s%d,%s10", asmInstructionsName[asmInstr].c_str(),
              getRegType(asmInstr, 0).c_str(), register_numl.get(),
              getRegType(asmInstr, 1).c_str(), register_numl.get(),
              getRegType(asmInstr, 2).c_str()));
        }
      } else {
        if (ff) {
          bufferText->blankCurrent();
        ...
        bufferText->addAfter(string_format(
            ...
            getRegType(asmInstr, 0).c_str(), register_numl.get(),
            getRegType(asmInstr, 1).c_str(), register_numl.get(),
            getRegType(asmInstr, 2).c_str(), register_numl.get()));
      }
    }
  } break;
  case TokenKeywordAnd:
    bufferText->addAfter(string_format("and a%d,a%d,a%d", register_numl.get(),
                                       register_numl.get(),
                                       register_numr.get()));
    break;
  case TokenKeywordOr:
    bufferText->addAfter(string_format("or a%d,a%d,a%d", register_numl.get(),
                                       register_numl.get(),
                                       register_numr.get()));
    break;
  case TokenKeywordFabs:

    bufferText->addAfter(string_format("abs.s f%d,f%d", register_numl.get(),
                                       register_numl.get()));
    break;
  case TokenKeywordAbs:
    if (ff) {
      ...
      bufferText->addAfter(string_format("abs a%d,a%d", register_numl.get(),
                                         register_numl.get()));
    }
    break;
  case TokenNegation:
    ...
    bufferText->addAfter(
        string_format("%s %s%d,%s%d", asmInstructionsName[asmInstr].c_str(),
                      getRegType(asmInstr, 0).c_str(), register_numl.get(),
                      getRegType(asmInstr, 1).c_str(), register_numl.get()));
    bufferText->sp.pop();
    bufferText->sp.push(bufferText->get());
...
void _visitglobalVariableNode(NodeToken *nd) {
  ...
  if (nd->children_size() > 0) {
    ...
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      nd->getChildAtPos(i)->visitNode();
      register_numl.pop();
      translateType(__int__, nd->getChildAtPos(i)->getVarType()->_varType,
                    register_numl.get());
      if (nd->children_size() > 1) {
        if (i < nd->children_size() - 1) {
          ...
          for (int h = 1; h < nd->children_size() - i; h++) {
            bufferText->addAfter(string_format(
                "movi a11,%d", stringToInt((char *)tile[i + 1 + h].c_str())));
            // bufferText->addAfter(string_format("mull a11,a10,a11"));
            bufferText->addAfter(string_format(
                "mull a%d,a%d,a11", register_numl.get(), register_numl.get()));
          }
          // if(i>0)
          bufferText->addAfter(
              string_format("add a10,a10,a%d", register_numl.get()));
        } else {
          bufferText->addAfter(string_format(
              "add a%d,a10,a%d", register_numl.get(), register_numl.get()));
        }
      }
  ...
  // string body = "";
  //  register_numl++;
  //  bufferText->addAfter(string_format("l32r a%d,%s", point_regnum,
  //  nd->_token->text.c_str()));
  if (nd->isPointer && nd->children_size() > 0) // leds[g];
  {
    // f=f+number.f;
    if (nd->type == TokenUserDefinedVariableMember or
        nd->type == TokenUserDefinedVariableMemberFunction) {
      bufferText->addAfter(
          string_format("movi a%d,%d", point_regnum, nd->_total_size));
      bufferText->addAfter(string_format("mull a%d,a%d,a%d",
                                         register_numl.get(),
                                         register_numl.get(), point_regnum));
      bufferText->addAfter(
          string_format("l32r a%d,@_%s", point_regnum, nd->getTokenText()));
      bufferText->addAfter(string_format("add a%d,a%d,a%d", point_regnum,
                                         point_regnum, register_numl.get()));
    } else if (v->total_size > 4) {
      // string tmp=content.l->back();
      // content.l->pop_back();
      bufferText->addAfter(
          string_format("movi a%d,%d", point_regnum, v->total_size));
      bufferText->addAfter(string_format("mull a%d,a%d,a%d",
                                         register_numl.get(),
                                         register_numl.get(), point_regnum));
      bufferText->addAfter(
  ...
  {
    bufferText->addAfter(
        string_format("mov a%d,a%d", register_numl.get(), point_regnum));
    bufferText->sp.push(bufferText->get());
  } else if ((nd->children_size() > 0 or !nd->isPointer) &&
             nd->type !=
                 TokenUserDefinedVariableMemberFunction) // leds[h] or h h being
                                                         // global)
  {
    // if (nd->target == EOF_TEXTARRAY)
    //  {
    for (int i = 0; i < v->size; i++) {
      // bufferText->addAfter(string_format("%s %s%d,%s%d,%d",
      // v->load[i].c_str(), v->reg_name.c_str(), register_numl.get(),
      // v->reg_name.c_str(), point_regnum, start));
      asmInstruction asmInstr = v->load[i];
      bufferText->addAfter(string_format(
          "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
          getRegType(asmInstr, 0).c_str(), register_numl.get(),
          getRegType(asmInstr, 1).c_str(), point_regnum, start));
      // register_numl--;
      start += v->sizes[i];
      bufferText->sp.push(bufferText->get());
    }
     ...
     else
     {
         ...
         if (i > -1)
         {
             ...
             for (int l = pos; l <= v->memberSize[i] - 1 + pos; l++)
             {
                 ...
                 bufferText->addAfter(string_format("%s %s%d,%s%d,%d",
     asmInstructionsName[v->load[l]].c_str(), getRegType(v->load[l], 0).c_str(),
     register_numl.get(), getRegType(v->load[l], 1).c_str(), point_regnum,
     start)); start += v->sizes[l]; bufferText->sp.push(bufferText->get());
             }
         }
         else
         {
...
void _visitcallConstructorNode(NodeToken *nd) {
  ...
  bufferText->addAfter("mov a10,a5");
  // bufferText->addAfter("mov a10,a2");
  bufferText->addAfter(string_format("call8 @_%s._@%s()",
                                     nd->getVarType()->varName.c_str(),
                                     nd->getVarType()->varName.c_str()));
  if (size > 1) {
    bufferText->addAfter(
        string_format("addi a5,a5,%d", nd->getVarType()->total_size));
...
void _visitlocalVariableNode(NodeToken *nd) {
  ...
  if (nd->asPointer) {
    ...
    if (nd->type == TokenUserDefinedVariableMember or
        nd->type == TokenUserDefinedVariableMemberFunction) {
      if (!nd->isPointer) {
        bufferText->addAfter(string_format("addi a%d,a1,%d",
                                           register_numl.get(),
                                           start - (int)(start / 1000) * 1000));
        bufferText->addAfter(
            string_format("l32i a%d,a%d,%d", register_numl.get(),
                          register_numl.get(), (start / 1000)));
      } else {
        // bufferText->addAfter(string_format("l32i a%d,a1,%d",
        // register_numl.get(),start-(int)(start/1000)*1000));
        asmInstruction asmInstr = v->load[0];
        // bufferText->addAfter(string_format("%s %s%d,%s%d,%d",
        // asmInstructionsName[asmInstr].c_str(), getRegType(asmInstr,
        // 0).c_str(), register_numl.get(), getRegType(asmInstr, 1).c_str(),
        // register_numl.get(), start/1000));
        bufferText->addAfter(string_format(
            "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
            getRegType(asmInstr, 0).c_str(), register_numl.get(),
            getRegType(asmInstr, 1).c_str(), 2, start / 1000));
        translateType(globalType.get(), v->_varType, register_numl.get());
        // bufferText->addAfter(string_format("l16si a%d,a%d,%d",
        // register_numl.get(),register_numl.get(),start/1000));
      }
    } else {
      bufferText->addAfter(
          string_format("addi a%d,a1,%d", register_numl.get(), start));
    }
    register_numl.decrease();
    if (nd->isPointer && nd->children_size() > 0) {
      ...
      for (int i = 0; i < v->total_size; i++) {
        bufferText->addAfter(
            string_format("add a%d,a%d,a%d", register_numl.get() + 1,
                          register_numl.get() + 1, register_numl.get()));
      }
      register_numl.increase();
    }
    bufferText->sp.push(bufferText->get());
  ...
  // if (nd->children_size() > 0)
  //{
  //  number = nd->getChildAtPos(0)->visitNode(nd->getChildAtPos(0),
  //  register_numl, register_numr);
  // }
  ...
  if (nd->isPointer) {
    ...
    // bufferText->addAfter(string_format("addi a%d,a1,%d", point_regnum,
    // start));
    ...
    if (nd->children_size() == 0) {
      if (nd->type == TokenUserDefinedVariableMemberFunction)
        ...
        bufferText->addAfter(
            string_format("l32i a%d,a1,%d", register_numl.get(), start));
      // bufferText->addAfter(string_format("mov a%d,a%d", register_numl.get(),
      // point_regnum));
      bufferText->sp.push(bufferText->get());
    } else {
      bufferText->addAfter(
          string_format("l32i a%d,a1,%d", point_regnum, start));
      start = 0;
      for (int i = 0; i < v->total_size; i++) {
        bufferText->addAfter(string_format("add a%d,a%d,a%d", point_regnum,
                                           point_regnum, register_numl.get()));
    ...
    for (int i = 0; i < v->size; i++) {
      ...
      bufferText->addAfter(string_format(
          "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
          getRegType(asmInstr, 0).c_str(), register_numl.get(),
          getRegType(asmInstr, 1).c_str(), regnum, start));
      // printf("jj2 :%s\n",string_format("%s %s%d,%s%d,%d",
      // asmInstructionsName[asmInstr].c_str(), getRegType(asmInstr, 0).c_str(),
      // register_numl.get(), getRegType(asmInstr, 1).c_str(), regnum,
      // start).c_str());
      translateType(globalType.get(), v->_varType, register_numl.get());
      // printf("jj3\n");
      // register_numl--;
...
void _visitdefFunctionNode(NodeToken *nd) {
  ...
  if (!isStructFunction) {
    string variables = "";
    for (int i = 0; i < nd->getChildAtPos(1)->children_size(); i++) {
      variables = string_format(
          "%s %d", variables.c_str(),
          nd->getChildAtPos(1)->getChildAtPos(i)->getVarType()->total_size);
    }
    header.addAfter(string_format(
        ".var %d%s", nd->getChildAtPos(1)->children_size(), variables.c_str()));
  }
  if (nd->getChildAtPos(1)->children_size() > -1) {
    header.addAfter(string_format("@_stack__%s:", nd->getTokenText()));
    header.addAfter(string_format(
        ".bytes %d", (nd->getChildAtPos(1)->children_size() + 1) * 4));
  }
  if (!isStructFunction) {
    ...
    if (variaToken->children_size() > 0) {
      bufferText->addAfter(string_format(
          "entry a1,%d", ((nd->stack_pos) / 8 + 1) * 8 + 16 +
                             _STACK_SIZE)); // ((nd->stack_pos) / 8 + 1) * 8+20)
      bufferText->addAfter(
          string_format("l32r a9,@_stack__%s", nd->getTokenText()));
    }
    for (int k = 0; k < variaToken->children_size(); k++) {
      ...
      // printf("ee p\r\n");
      int start = variaToken->getChildAtPos(k)->stack_pos;
      for (int j = 0; j < variaToken->getChildAtPos(k)->getVarType()->size;
           j++) {
        asmInstruction asmInstr =
            variaToken->getChildAtPos(k)->getVarType()->load[0];
        bufferText->addAfter(string_format(
            "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
            getRegType(asmInstr, 0).c_str(), k + 10,
            getRegType(asmInstr, 1).c_str(), 9,
            start -
                _STACK_SIZE)); // point_regnum
                               // asmInstruction asmInstr =
                               // variaToken->getChildAtPos(k)->getVarType()->store[0];
        //                   bufferText->addAfter(string_format("%s
        //                   %s%d,%s9,%d",
        //                   asmInstructionsName[asmInstr].c_str(),
        //                   getRegType(asmInstr, 0).c_str(),
        //                   k+10,getRegType(asmInstr, 1).c_str(), start));
...
#if _TRIGGER == 0
  bufferText->addAfterNoDouble(
      string_format("l32r a%d,@_stack_%s", sav, nd->getTokenText()));
#endif
  if (saveReg) {
...
void _visitprogramNode(NodeToken *nd) {
  ...
  for (int i = 0; i < nd->children_size(); i++) {
    ...
    if (nd->getChildAtPos(i)->_nodetype != defFunctionNode &&
        nd->getChildAtPos(i)->_nodetype != defAsmFunctionNode) {
#endif
      nd->getChildAtPos(i)->visitNode();
#ifndef __MEM_PARSER
    } // NEW
...
void _visitassignementNode(NodeToken *nd) {
  ...
  if (nd->children_size() > 1) {
    ...
    if (nd->getChildAtPos(1)->getVarType() != NULL) {
      ...
      translateType(globalType.get(),
                    nd->getChildAtPos(1)->getVarType()->_varType,
                    register_numl.get());
...
void _visitternaryIfNode(NodeToken *nd) {
  ...
  register_numl.pop();
  bufferText->addAfter(
      string_format("beqz a%d,%s", register_numl.get(), nd->getTargetText()));
  /*
          register_numr.clear();
  register_numl.clear();
  register_numl.push(15);
...
void _visittestNode(NodeToken *nd) {
  ...
  string _add = "";
  if (nd->getChildAtPos(0)->_vartype == __uint32_t__ ||
      nd->getChildAtPos(1)->_vartype == __uint32_t__)
    _add = "u";
  // register_numl.duplicate();
  nd->getChildAtPos(0)->visitNode();
  // register_numl.pop();
  ...
  //////printf("compare %s %s\n",tokenNames[nd->_token->type
  ///].c_str(),nd->_token->text.c_str());
  string compop = "";
  string compo2 = "";
  // to compose
  int h = 999;
  ...
  if (nd->getChildAtPos(1)->_vartype == __float__) {
    ...
    bufferText->addAfter(
        string_format("%s b0,f%d,f%d", compop.c_str(), numl, leftl));
    bufferText->addAfter(
        string_format("%s b0,%s_end", compo2.c_str(), nd->getTargetText()));
    ...
    bufferText->addAfter(string_format("%s%s a%d,a%d,%s_end", compop.c_str(),
                                       _add.c_str(), numl, leftl,
                                       nd->getTargetText()));
    bufferText->addAfter(string_format("movi a%d,1", h));
    bufferText->addAfter(string_format("j %s_end_", nd->getTargetText()));
    bufferText->addAfter(string_format("%s_end:", nd->getTargetText()));
...
void _visitcomparatorNode(NodeToken *nd) {
  ...
  if (nd->getChildAtPos(0)->_nodetype != testNode) {
    ...
    if (nd->_total_size > 116) {
      bufferText->addAfter(
          string_format("bnez a%d,%s_if", numl, nd->parent->getTargetText()));
      bufferText->addAfter(
          string_format("j %s_end", nd->parent->getTargetText()));
      bufferText->addAfter(
          string_format("%s_if:", nd->parent->getTargetText()));
      register_numl.increase();
    } else {
      bufferText->addAfter(
          string_format("beqz a%d,%s_end", numl, nd->parent->getTargetText()));
      // bufferText->addAfter(_compare.back()+1,string_format("j %s_end",
      // nd->target.c_str()));
      // bufferText->addAfter(_compare.back()+2,string_format("%s_if:",
      // nd->target.c_str()));
      register_numl.increase();
    }
  ...
  if (nd->getChildAtPos(1)->_vartype == __float__) {
    ...
    if (nd->_total_size > 116) {
      ...
      bufferText->addAfter(
          string_format("%s b0,f%d,f%d", compop.c_str(), numl, leftl));
      bufferText->addAfter(
          string_format("%s b0,%s_if", compo2.c_str(), nd->getTargetText()));
      bufferText->addAfter(string_format("j %s_end", nd->getTargetText()));
      ...
      bufferText->addAfter(
          string_format("%s b0,%s_end", compo2.c_str(), nd->getTargetText()));
      // bufferText->addAfter(string_format("%s a%d,a%d,%s_end", compop.c_str(),
      // numl, leftl, nd->getTargetText()));
    ...
    if (nd->_total_size > 127) {
      ...
      bufferText->addAfter(string_format("%s%s a%d,a%d,%s_if", compop.c_str(),
                                         _add.c_str(), numl, leftl,
                                         nd->getTargetText()));
      bufferText->addAfter(string_format("j %s_end", nd->getTargetText()));
      bufferText->addAfter(string_format("%s_if:", nd->getTargetText()));
...
void _visitCallFunctionTemplate(NodeToken *nd, int regbase, bool isExtCall) {
  ...
  // printf(" %s %d %d\n",nd->_token->text.c_str(),
  // t->children_size(),t->getChildAtPos(1)->children_size()); for (int i = 0; i
  // < t->getChildAtPos(1)->children_size(); i++)

  // printf("number of arg %s %d\r\n", nd->getTokenText(),
  // nd->findMaxArgumentSize());
  for (int i = t->getChildAtPos(0)->children_size() - 1; i >= 0; i--) {
    // printf("***number of arg %d %d\r\n", i,
    // nd->getChildAtPos(2)->getChildAtPos(i)->findMaxArgumentSize());
    ...
    if (t->getChildAtPos(0)->getChildAtPos(i)->isPointer) {
      ...
      if (save_in_stack == true) {
        bufferText->addAfter(string_format(
            "s32i a%d,a1,%d", register_numl.get(), i * 4 + _START_2));
      } else {
        bufferText->addAfter(
            string_format("mov a%d,a%d", regbase + i, register_numl.get()));
      }
    } else

    {
      if (i < func->getChildAtPos(1)->children_size()) {
      ...
      if (_vartype == __float__) {
        ...
        if (isArg) {
          ...
          if (save_in_stack == true) {
            bufferText->addAfter(string_format(
                "ssi f%d,a1,%d", register_numl.get(), i * 4 + _START_2));
          } else {
            bufferText->addAfter(
                string_format("rfr a%d,f%d", regbase + i, register_numl.get()));
          }
        }
      } else if (l->getChildAtPos(i)->getVarType()->_varType == __CRGB__ or
                 l->getChildAtPos(i)->getVarType()->_varType == __CRGBW__) {
        // bufferText->addAfter( bufferText->sp.pop(),string_format("mov
        // a%d,a%d", 10 + i, register_numl.get()));
        if (t->getChildAtPos(0)
                ->getChildAtPos(i)
                ->getChildAtPos(0)
          ...
          if (save_in_stack == true) {
            for (int k = 0; k < 3; k++) {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("s8i a%d,a1,%d",
                                                 register_numl.get(),
                                                 i * 4 + _START_2 + k));
            }
            // bufferText->addAfter(string_format("l32i a%d,a1,%d",10+i,i * 4 +
            // _START_2));
          }
            ...
            bufferText->addAfter(
                string_format("l32i a%d,a1,%d", regbase + i, i * 4 + _START_2));
          }
        } else {

          for (int k = 0; k < t->getChildAtPos(0)
                                  ->getChildAtPos(i)
                                  ->getChildAtPos(0)
                                  ->getVarType()
                                  ->size;
            ...
            // bufferText->addAfter(bufferText->sp.pop(),string_format("slli
            // a%d,a%d,%d", 10+i,register_numl.get(),  k* 8)); register_numl--;
            bufferText->pop();
          }
          if (t->getChildAtPos(0)
                  ->getChildAtPos(i)
            ...
            if (save_in_stack == true) {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("l32i a%d,a8,0", regbase + i));
              bufferText->addAfter(string_format("s32i a%d,a1,%d", regbase + i,
                                                 i * 4 + _START_2));
            } else {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("l32i a%d,a8,0", 10 + i));
            }
          }
          if (t->getChildAtPos(0)
                  ->getChildAtPos(i)
            ...
            if (save_in_stack == true) {
              ...
              // a%d,a10", 10 + i));
              bufferText->addAfter(
                  string_format("s32i a10,a1,%d", i * 4 + _START_2));
            } else {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("mov a%d,a10", regbase + i));
            }
          } else if (t->getChildAtPos(0)
                         ->getChildAtPos(i)
                         ->getChildAtPos(0)
                         ->_nodetype == localVariableNode) {
            if (save_in_stack == true) {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("l32i a%d,a1,%d", regbase + i,
                                                 t->getChildAtPos(0)
                                                     ->getChildAtPos(i)
                                                     ->getChildAtPos(0)
                                                     ->stack_pos));
            ...
            if (save_in_stack == true) {
              bufferText->addAfter(bufferText->sp.pop(),
                                   string_format("l32i a%d,a5,0", regbase + i));
              bufferText->addAfter(string_format("s32i a%d,a1,%d", regbase + i,
                                                 i * 4 + _START_2));
            } else {
              bufferText->addAfter(bufferText->sp.pop(),
        ...
        if (save_in_stack == true) {
          ...
          if (i >= 6) {
            bufferText->addAfter(string_format(
                "s32i a%d,a1,%d", register_numl.get(), staack_offset));
            staack_offset -= 4;
          } else {
            bufferText->addAfter(
                string_format("mov a%d,a%d", regbase + i, register_numl.get()));
    ...
    if (i < 7) {
      if (saveinstack[i] == true) {
        ...
        if (v->_varType == __float__) {
          ...
          bufferText->addAfter(string_format(
              "%s %s%d,%s1,%d", asmInstructionsName[v->load[0]].c_str(),
              getRegType(v->load[0], 0).c_str(), regbase + i,
              getRegType(v->load[0], 1).c_str(), i * 4 + _START_2));
        }
      }
    }
  }
...
void _visitcallFunctionNode(NodeToken *nd) {
  ...
  NodeToken *t = nd; // cntx.findFunction(nd->_token);
  NodeToken *func = _functions[nd->target];
  if (func->getChildAtPos(1)->children_size() >=
      _TRIGGER) { //  printf("token type
    ...
    int save = 9; // point_regnum;
                  ...
                  // a%d,@_stack_%s", save,nd->getTokenText())); // point_regnum
    if (func->getChildAtPos(1)->children_size() == 0 and
        t->type == TokenUserDefinedVariableMemberFunction) {
    ...
    for (int i = 0; i < func->getChildAtPos(1)->children_size(); i++) {
      ...
      if (func->getChildAtPos(1)->getChildAtPos(i)->isPointer) {
        ...
        if (t->type == TokenUserDefinedVariableMemberFunction and i == 0) {
          ...
          if (!isStructFunction) {
            ...
            // bufferText->pop();
            bufferText->addAfter(bufferText->sp.pop(),
                                 string_format("mov a10,a2"));
          }
        } else {
          bufferText->addAfter(bufferText->sp.pop(),
                               string_format("s32i a%d,a%d,%d",
                                             register_numl.get(), save,
                                             start)); // point_regnum
          bufferText->addBefore(string_format(
              "l32r a%d,@_stack_%s", save, nd->getTokenText())); // point_regnum
        } // isPointer=false;
      } else {
        ...
        for (int j = 0;
          ...
          if (nd->getChildAtPos(0)->getChildAtPos(i)->getVarType() != NULL) {
            ...
            //   translateType(globalType.get(),
            //   nd->getChildAtPos(0)->getChildAtPos(i)->_token->_varType,
            //   register_numl.get());
          }

          start -= func->getChildAtPos(1)
                       ->getChildAtPos(i)
                       ->getVarType()
                       ->sizes[tot - j];
          asmInstruction asmInstr = func->getChildAtPos(1)
                                        ->getChildAtPos(i)
                                        ->getVarType()
                                        ->store[tot - j];
          // bufferText->addAfter(bufferText->sp.pop(), string_format("%s
          // %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
          // getRegType(asmInstr, 0).c_str(), register_numl.get(),
          // getRegType(asmInstr, 1).c_str(), point_regnum, start));
          int sav;
          // if (j == t->getChildAtPos(1)->getChildAtPos(i)->getVarType()->size
          // - 1)
          if (!intest) {
            sav = bufferText->get();
            bufferText->addAfter(
                ...
                string_format(
                    "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
                    getRegType(asmInstr, 0).c_str(), register_numl.get(),
                    getRegType(asmInstr, 1).c_str(), save, start));
            if (j ==
            ...
            if (j ==
              ...
              bufferText->addBefore(string_format(
                  "l32r a%d,@_stack_%s", save,
                  nd->getTokenText())); // point_regnum
                                        // bufferText->putIteratorAtPos(bufferText->get()+1);
                                        // // bufferText->addAfter(sav,
                                        // string_format("l32r
                                        // a%d,@_stack",save));
            }
      ...
      // a%d,a1,%d",11+i,t->getChildAtPos(1)->getChildAtPos(i)->stack_pos));
    }
    // bufferText->addAfter(string_format("mov a10,a2")); // neded to find the
    // external variables !!!!!!
    bufferText->addAfter(string_format("call8 @_%s", nd->getTokenText()));
  } else {
  ...
  int start = nd->stack_pos;
  // printf("ini\r\n");
  varType *v = func->getChildAtPos(0)->getVarType();
  // printf("ini\r\n");
  if (v == NULL) {
    printf("nodeToken %d\r\n", func->getChildAtPos(0)->type);
    printf("NULL\r\n");
  }
  ...
  if (v->size > 1) {
    ...
    for (int i = 0; i < v->size; i++) {
      ...
      bufferText->addAfter(string_format(
          "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
          getRegType(asmInstr, 0).c_str(), register_numl.get(),
          getRegType(asmInstr, 1).c_str(), 8, start)); // point_regnum
      // register_numl--;
      start += v->sizes[i];
      // bufferText->sp.push(bufferText->get());
    }
...
void _visitextGlobalVariableNode(NodeToken *nd) {
  ...
  if (!nd->isPointer) {
    bufferText->addAfter(
        string_format("movExt a%d,@_ext_%s", point_regnum, nd->getTokenText()));
  } else {
    if (nd->children_size() > 0) {
      ...
      if (nd->children_size() > 1) {
        ...
        for (int par = 0; par < nd->children_size(); par++)
        {
            ...
            register_numl.pop();
            bufferText->addAfter(string_format("mov a%d,a%d", 10 + par,
        register_numl.get()));
        }
        bufferText->addAfter(string_format("callExt a%d,map%dD",
        register_numl.get(), nd->children_size()));
        bufferText->addAfter(string_format("mov a%d,a10", register_numl.get()));
    */
        vector<string> tile;
        int nb = 0;
        ...
        for (int par = 0; par < nd->children_size(); par++) {
          ...
          if (nd->getChildAtPos(par)->getVarType() != NULL) {
            translateType(__int__,
                          nd->getChildAtPos(par)->getVarType()->_varType,
                          register_numl.get());
          }
          if (nb > 1) {
            if (par < nd->children_size() - 1) {
              ...
              for (int h = 1; h < nd->children_size() - par; h++) {
                bufferText->addAfter(string_format(
                    "movi a11,%d",
                    stringToInt((char *)tile[par + 1 + h].c_str())));
                // bufferText->addAfter(string_format("mull a11,a10,a11"));
                bufferText->addAfter(string_format("mull a%d,a%d,a11",
                                                   register_numl.get(),
                                                   register_numl.get()));
              }
              bufferText->addAfter(
        ...
        translateType(__int__, nd->getChildAtPos(0)->getVarType()->_varType,
                      register_numl.get());
      }

      bufferText->addAfter(string_format("movExt a%d,@_ext_%s", point_regnum,
                                         nd->getTokenText()));
      // f=f+number.f;
      for (int i = 0; i < v->total_size; i++) {
        bufferText->addAfter(string_format("add a%d,a%d,a%d", point_regnum,
      ...
      bufferText->addAfter(string_format(
          "movExt a%d,@_ext_%s", register_numl.get(), nd->getTokenText()));
      bufferText->sp.push(bufferText->get());
      bufferText->sp.push(bufferText->get());
      register_numl.pop();
      register_numl.decrease();
  ...
  bufferText->sp.push(bufferText->get());
  for (int i = 0; i < v->size; i++) {
    // bufferText->addAfter(string_format("%s %s%d,%s%d,%d", v->load[i].c_str(),
    // v->reg_name.c_str(), register_numl.get(), v->reg_name.c_str(),
    // point_regnum, start));
    asmInstruction asmInstr = v->load[i];
    bufferText->addAfter(
        string_format("%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
                      getRegType(asmInstr, 0).c_str(), register_numl.get(),
                      getRegType(asmInstr, 1).c_str(), point_regnum, start));
    // register_numl--;
    start += v->sizes[i];
...
void _visitextCallFunctionNode(NodeToken *nd) {
  ...
  if (v->_varType == __float__) {
    ...
    if (v->size > 1) {
      for (int i = 0; i < v->size; i++) {
        // bufferText->addAfter(string_format("mov a15,a10"));
        bufferText->addAfter(string_format("extui a%d,a%d,%d,%d",
                                           register_numl.get(), 10, start * 8,
                                           v->sizes[i] * 8));
        // register_numl--;
        start += v->sizes[i];
        bufferText->sp.push(bufferText->get());
...
void _visitdefInputArgumentsNode(NodeToken *nd) {
  ...
  int sav = 9;
  if (nd->children_size() >=
      _TRIGGER) { // point_regnum;
                  // 17/01 // bufferText->addAfterNoDouble(string_format("l32r
                  // a%d,@_stack_%s", sav, nd->parent->getTokenText())); //
                  // point_regnum
                  // bufferText->addAfterNoDouble(string_format("l32r
                  // a%d,@_stack", sav));
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      if (nd->getChildAtPos(i)->isPointer) {
        ...
        int start = nd->getChildAtPos(i)->stack_pos;
        bufferText->addAfter(string_format(
            "l32i a15,a%d,%d", sav, start - _STACK_SIZE)); // point reg_bnum
        ...
        for (int j = 0; j < nd->getChildAtPos(i)->getVarType()->size; j++) {
          ...
          bufferText->addAfter(string_format(
              "%s %s15,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
              getRegType(asmInstr, 0).c_str(), getRegType(asmInstr, 1).c_str(),
              sav, start - _STACK_SIZE)); // point_regnum
          asmInstr = nd->getChildAtPos(i)->getVarType()->store[0];
          bufferText->addAfter(string_format(
              "%s %s15,%s1,%d", asmInstructionsName[asmInstr].c_str(),
              getRegType(asmInstr, 0).c_str(), getRegType(asmInstr, 1).c_str(),
              start));
    ...
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      if (reg_num <= 7) {
        ...
        // printf("ee p\r\n");
        //  int start = nd->getChildAtPos(i)->stack_pos;
        // bufferText->addAfter(string_format("l32i a15,a%d,%d", sav, start -
        // _STACK_SIZE)); // point reg_bnum
        //
        if (nd->getChildAtPos(i)->isPointer) {
          bufferText->addAfter(string_format("s32i a%d,a1,%d", reg_num, start));
        } else if (nd->getChildAtPos(i)->getVarType()->_varType == __float__) {
          bufferText->addAfter(string_format("s32i a%d,a1,%d", reg_num, start));
          // bufferText->addAfter(string_format("%s %s15,%s1,%d",
          // asmInstructionsName[asmInstr].c_str(), getRegType(asmInstr,
          // 0).c_str(), getRegType(asmInstr, 1).c_str(), start));
        } else {
          asmInstruction asmInstr =
              nd->getChildAtPos(i)->getVarType()->store[0];
          bufferText->addAfter(string_format(
              "%s a%d,a1,%d", asmInstructionsName[asmInstr].c_str(), reg_num,
              start));
        }
        reg_num++;
      } else {
        asmInstruction asmInstr = nd->getChildAtPos(i)->getVarType()->load[0];
        bufferText->addAfter(
            string_format("%s a15,a1,%d", asmInstructionsName[asmInstr].c_str(),
                          stek + start - nd->getChildAtPos(6)->stack_pos));
        asmInstr = nd->getChildAtPos(i)->getVarType()->store[0];
        bufferText->addAfter(string_format(
            "%s a15,a1,%d", asmInstructionsName[asmInstr].c_str(), start));
      }
      /*
      }
      else
      {
         // printf("ee j\r\n");
          ...
          {
              asmInstruction asmInstr =
      nd->getChildAtPos(i)->getVarType()->load[0];
              bufferText->addAfter(string_format("%s %s15,%s%d,%d",
      asmInstructionsName[asmInstr].c_str(), getRegType(asmInstr, 0).c_str(),
      getRegType(asmInstr, 1).c_str(), sav, start - _STACK_SIZE)); //
      point_regnum asmInstr = nd->getChildAtPos(i)->getVarType()->store[0];
              bufferText->addAfter(string_format("%s %s15,%s1,%d",
      asmInstructionsName[asmInstr].c_str(), getRegType(asmInstr, 0).c_str(),
      getRegType(asmInstr, 1).c_str(), start)); start +=
      nd->getChildAtPos(i)->getVarType()->sizes[j];
          }
      }
      */
    }
...
void _visitdefExtGlobalVariableNode(NodeToken *nd) {
  if (safeMode) {
    if (nd->isPointer) {
      ...
      header.addAfter(
          _numToBytes(nd->_total_size / nd->getVarType()->total_size).c_str());
    }
  }
  header.addAfter(string_format("@_ext_%s:", nd->getTokenText()));
  header.addAfter(".bytes 4");
}
void _visitdefGlobalVariableNode(NodeToken *nd) {
  if (strcmp(nd->getTokenText(), "_handle_") == 0 ||
      strcmp(nd->getTokenText(), "_execaddr_") == 0)
    return;
  if (safeMode) {
    if (nd->isPointer) {
      header.addAfter(string_format("@_size_%s:", nd->getTokenText()));
  ...
  if (nd->children_size() == 0) {
    ...
    _data_sav = "";
    if (nd->getVarType()->_varType == __CRGB__ or
        nd->getVarType()->_varType == __CRGBW__) {
      if (nd->children_size() > 0) {
      ...
      nd->_total_size =
          str.size() - 1; // we remove the size taken by " " adn we add 0 fo \0
    ...
    header.addAfter(
        string_format(".bytes %d%s", nd->_total_size, _data_sav.c_str()));
  }
}
void _visitdefLocalVariableNode(NodeToken *nd) {}
...
void _visitstoreLocalVariableNode(NodeToken *nd) {
  ...
  if (nd->children_size() > 0 or !nd->isPointer or nd->asPointer) {
    for (int i = v->size - 1; i >= 0; i--) {
      ...
      // bufferText->addAfter(  bufferText->sp.pop(),  string_format("%s
      // %s%d,%s%d,%d", v->store[i].c_str(), v->reg_name.c_str(), numl,
      // v->reg_name.c_str(), regnum, start));
      bufferText->addAfter(
          bufferText->sp.pop(),
          string_format("%s %s%d,%s%d,%d",
                        asmInstructionsName[v->store[i]].c_str(),
                        getRegType(v->store[i], 0).c_str(), numl,
                        getRegType(v->store[i], 1).c_str(), regnum, start));
      // numl++;
      start -= v->sizes[i];
    }
  } else {
    bufferText->addAfter(
        bufferText->sp.pop(),
        string_format("s32i a%d,a%d,%d", numl, regnum, 0)); // start
    // bufferText->addAfter(bufferText->sp.pop(), string_format("s32i
    // a%d,a%d,%d", numl, 1,nd->stack_pos));
  }
  if (nd->isPointer) {
    register_numl.duplicate();
    bufferText->sp.push(bufferText->get());
    ...
    if (nd->children_size() > 0) {
      bufferText->addAfter(
          string_format("l32i a%d,a1,%d", point_regnum, nd->stack_pos));
      for (int i = 0; i < v->total_size; i++) {
        bufferText->addAfter(string_format("add a%d,a%d,a%d", point_regnum,
                                           point_regnum, register_numl.get()));
      }
      ...
      if (nd->type == TokenUserDefinedVariableMember or
        ...
        if (isStructFunction) {
          bufferText->addAfter(string_format("addi a%d,a%d,%d", point_regnum, 2,
                                             (int)(nd->stack_pos / 1000)));
        } else {
          bufferText->addAfter(string_format(
              "l32i a%d,a1,%d", point_regnum,
              nd->stack_pos - (int)(nd->stack_pos / 1000) * 1000));
          bufferText->addAfter(string_format("addi a%d,a%d,%d", point_regnum,
                                             point_regnum,
                                             (int)(nd->stack_pos / 1000)));
        }
      } else
      ...
      // a remettre
      bufferText->addAfter(
          string_format("addi a%d,a1,%d", point_regnum, nd->stack_pos));
    }
    bufferText->end();
    bufferText->sp.pop();
    // point_regnum++;
...
void _visitstoreGlobalVariableNode(NodeToken *nd) {
  ...
  if ((nd->children_size() > 0 or !nd->isPointer)) {
    ...
    for (int i = v->size - 1; i >= 0; i--) {
      ...
      // bufferText->addAfter(bufferText->sp.pop(), string_format("%s
      // %s%d,%s%d,%d", v->store[i].c_str(), v->reg_name.c_str(),
      // register_numl.get(), v->reg_name.c_str(), point_regnum, start));
      bufferText->addAfter(
          bufferText->sp.pop(),
          string_format(
              "%s %s%d,%s%d,%d", asmInstructionsName[v->store[i]].c_str(),
              getRegType(v->store[i], 0).c_str(), register_numl.get(),
              getRegType(v->store[i], 1).c_str(), point_regnum, start));
      // register_numl--;
      // printf("here in store %d\r\n",i);
      start -= v->sizes[i - 1];
      // bufferText->sp.push(bufferText->get());
    ...
    bufferText->addAfter(bufferText->sp.pop(),
                         string_format("s32i a%d,a%d,%d", register_numl.get(),
                                       point_regnum, start));
  }
  // res.f = f;
  // res.header = number.header + h;
  ...
  if (safeMode && nd->isPointer) {
    bufferText->addAfter(
        string_format("l32r a%d,@_size_%s", point_regnum, nd->getTokenText()));
    bufferText->addAfter(
        string_format("l32i a%d,a%d,0", point_regnum, point_regnum));
    bufferText->addAfter(string_format("bge a%d,a%d,__test_safe_%d",
                                       point_regnum, register_numl.get(),
                                       for_if_num));
  ...
  bufferText->end();
  if (isStructFunction) {
    bufferText->addAfter(string_format("s32i a2,a1,%d", _STACK_SIZE));
  }
  bufferText->sp.pop();
  // if(!nd->isPointer)
  // point_regnum++;
...
void _visitstoreExtGlocalVariableNode(NodeToken *nd) {
  ...
  if (nd->children_size() == 1) {
    ...
    if (nd->getChildAtPos(0)->getVarType() != NULL) {
      ...
      // translateType(__int__, nd->getChildAtPos(0)->_token->_varType,
      // register_numl.get());
    }
...
*/
    ...
    for (int par = 0; par < nd->children_size(); par++) {
      ...
      nd->getChildAtPos(par)->visitNode();
      register_numl.pop();
      translateType(__int__, nd->getChildAtPos(par)->getVarType()->_varType,
                    register_numl.get());
      if (nb > 1) {
        if (par < nd->children_size() - 1) {
          ...
          for (int h = 1; h < nd->children_size() - par; h++) {
            bufferText->addAfter(string_format(
                "movi a11,%d", stringToInt((char *)tile[par + 1 + h].c_str())));
            // bufferText->addAfter(string_format("mull a11,a10,a11"));
            bufferText->addAfter(string_format(
                "mull a%d,a%d,a11", register_numl.get(), register_numl.get()));
          }
  ...
  if (safeMode && nd->isPointer) {
    bufferText->addAfter(
        string_format("l32r a%d,@_size_%s", regnum, nd->getTokenText()));
    bufferText->addAfter(string_format("l32i a%d,a%d,0", regnum, regnum));
    bufferText->addAfter(string_format("bge a%d,a%d,__test_safe_%d", regnum,
                                       register_numl.get(), for_if_num));
...
void _visitifNode(NodeToken *nd) {
  ...
  nd->getChildAtPos(0)->_total_size =
      (bufferText->get() - _compare.back()) *
      3; // on estime que toutes les instructions ont la meme taille
  bufferText->addAfter(string_format("%s_end:", nd->getTargetText()));
  // printf("oo3\n");
  bufferText->putIteratorAtPos(_compare.back());
  // bufferText->addAfter("");
...
void _visitreturnNode(NodeToken *nd) {
  ...
  if (t->getVarType()->size > 1) {
    ...
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      // bufferText->addBefore(bufferText->sp.get(),string_format("l32r
      // a%d,@_stackr", 8));
      for (int j = 0; j < t->getVarType()->size; j++) {
        ...
        bufferText->addAfter(
            ...
            string_format(
                "%s %s%d,%s%d,%d", asmInstructionsName[asmInstr].c_str(),
                getRegType(asmInstr, 0).c_str(), register_numl.get(),
                getRegType(asmInstr, 1).c_str(), 9, start)); // point_regnum
      }
      globalType.pop();
    }
  } else {
    for (int i = 0; i < nd->children_size(); i++) {
      ...
      nd->getChildAtPos(i)->visitNode();
      register_numl.pop();
      translateType(t->getVarType()->_varType,
                    nd->getChildAtPos(i)->getVarType()->_varType,
                    register_numl.get());
      // int start = nd->stack_pos + t->getVarType()->total_size;
      // int tot = t->getVarType()->size - 1;
      if (t->getVarType()->_varType == __float__) {
        // bufferText->addAfter(bufferText->sp.pop(), string_format("mov.s
        // f2,f%d", register_numl.get()));
        bufferText->addAfter(
            string_format("mov.s f2,f%d", register_numl.get()));
      } else {
        // bufferText->addAfter(bufferText->sp.pop(), string_format("mov
        // a2,a%d", register_numl.get()));
...
void _visitdefAsmFunctionNode(NodeToken *nd) {
  ...
  // bufferText->addAfter(string_format("entry a1,%d", 80)); // ((nd->stack_pos)
  // / 8 + 1) * 8)
...
void _visitstringNode(NodeToken *nd) {
  ...
  bufferText->addAfter(
      string_format("%s", target.substr(1, target.size() - 2).c_str()));
}
void _visitchangeTypeNode(NodeToken *nd) {
  ...
  for (int i = 0; i < nd->children_size(); i++) {
    ...
    if (nd->getChildAtPos(i)->getVarType() != NULL) {
      ...
      // translateType(globalType.get(), nd->getChildAtPos(i)->_token->_varType,
      // register_numl.get());
    }
    register_numl.decrease();
  }
...
void optimize(Text *text) {
          ...
          if (d[0].compare("call8") == 0 and regnum >= 8) {
            ...
            if (d[1].compare(0, registername.size(), registername) == 0) {
              ...
              //}
                ...
                if (d[0].compare("movi") == 0 or d[0].compare("l32i") == 0 or
                    d[0].compare("l32r") == 0 or d[0].compare("l16i") == 0 or
                    d[0].compare("l16ui") == 0 or d[0].compare("l8ui") == 0 or
                    d[0].compare("movExt") == 0) {
                  str = tmp;
                } else {
                  str = "__";
                }
  ...
  for (int i = 0; i < text->size(); i++) {
    ...
    if (tmp.compare(" ") != 0 && tmp != "") {
      ...
      } else if (d[0].find_first_of(":") != std::string::npos or
                 d[0].compare("call8") == 0 or d[0].compare("callExt") == 0) {
        before = " ";
        torep = " ";
      } else if (d.size() > 1) {
    ...
    if (*text->getChildAtPos(i) != NULL) {
      ...
      if (tmp.compare(" ") != 0 && tmp != "") {
        ...
        if (d2[0].compare("mov") == 0) {
          ...
          if (d[0].compare("mull") == 0 or d[0].compare("quou") == 0 or
              d[0].compare("quos") == 0 or d[0].compare("sub") == 0 or
              d[0].compare("add") == 0 or d[0].compare("mov") == 0 or
              d[0].compare("movi") == 0 or d[0].compare("l32i") == 0 or
              d[0].compare("l32r") == 0 or d[0].compare("l16i") == 0 or
              d[0].compare("l16ui") == 0 or d[0].compare("l8ui") == 0 or
              d[0].compare("addi") == 0) {
      ...
      if (tmp.compare(" ") != 0 && tmp != "") {
        ...
        if (index.size() > 0) {
          ...
          if (d[0].compare("bnez") == 0 or d[0].compare("beqz") == 0) {
            ...
            */
            // printf("new %s\n",newstr.c_str());
          } else if (d[0].compare("bge") == 0 or d[0].compare("blt") == 0 or
                     d[0].compare("beq") == 0 or d[0].compare("bne") == 0) {
            ...
            */
            // printf("new %s\n", newstr.c_str());
          } else if (d[0].compare("movi") == 0 or d[0].compare("rsr") == 0 or
                     d[0].compare("wsr") == 0 or d[0].compare("ssl") == 0) {
            d2 = split(d[1], ",");
            int ind;
            found = false;
          ...
          } else if (d[0].compare("neg") == 0 or d[0].compare("abs") == 0 or
                     d[0].compare("mov") == 0 or d[0].compare("sll") == 0 or
                     d[0].compare("srl") == 0) {
          ...
          else if (d[0].compare("round.s") == 0 or
                   d[0].compare("floor.s") == 0 or
                   d[0].compare("float.s") == 0 or d[0].compare("l32i") == 0 or
                   d[0].compare("s32i") == 0 or d[0].compare("l16i") == 0 or
                   d[0].compare("l16si") == 0 or d[0].compare("l16ui") == 0 or
                   d[0].compare("l8ui") == 0 or d[0].compare("s8i") == 0 or
                   d[0].compare("s16i") == 0 or d[0].compare("addi") == 0 or
                   d[0].compare("trunc.s") == 0) {
            d2 = split(d[1], ",");
            int ind;
            string newstr = d[0] + " " + d2[0] + ",";
            bool found = false;
            ...
            */
            //  printf("new %s\n", newstr.c_str());
          } else if (d[0].compare("remu") == 0 or d[0].compare("or") == 0 or
                     d[0].compare("and") == 0 or d[0].compare("mull") == 0 or
                     d[0].compare("sub") == 0 or d[0].compare("add") == 0 or
                     d[0].compare("quou") == 0 or d[0].compare("quos") == 0) {
            int ind = -1;
            ...
            */
                     ...
                     d[0].compare("call8") == 0 or
                     d[0].compare("callExt") == 0) {
