...
void addInstr(result_parse_line operande, parsedLines *asm_parsed) {
  if (operande.op != opCodeType::data && operande.op != opCodeType::number) {
    ...
    if (operande.nameref != EOF_TEXTARRAY) {
      ...
      memcpy(tmp_binary_data + _tmp_data_address, operande.getText(),
             operande.size);
...
result_parse_line *getInstrAtPos(int pos) {
  ...
  for (vector<result_parse_line *>::iterator it = _asm_parsed.begin();
...
class opHex {
public:
  static result_parse_operande parse(std::string s, uint32_t min,
                                     uint32_t max) {
    error_message_struct error;
    result_parse_operande res;
    char *endptr = NULL;
    ...
    error.error_message =
        string_format("incorrect value %s should be between %d and %d\n",
                      s.c_str(), min, max);
    error.error = 1;
    res.error = error;
    return res;
...
result_parse_line parseOperandes(string str, int nboperande,
  ...
  if (operandes.size() == nboperande) {
    ...
    asm_Error.error = 1;
    asm_Error.error_message =
        string_format("asm_Error:expected %d arguments got %d\n", nboperande,
                      operandes.size());
  }
...
int findLabel(string s, parsedLines *asm_parsed) {
  int res = -1;
  int i = 0;
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
       it != asm_parsed->end(); it++) {
    if ((*it)->op == opCodeType::label || (*it)->op == opCodeType::data_label ||
        (*it)->op == opCodeType::number_label ||
        (*it)->op == opCodeType::external_var_label ||
...
result_parse_line parseline(line sp, parsedLines *asm_parsed) {
  ...
  if (sp.opcde.find(":") != -1) {
    ...
    if (findLabel(string(res.getText()), asm_parsed) != -1) {
      asm_Error.error = 1;
      // res.error.error_message = string_format("label %s is already declared
      // in line %d\n", res.name.c_str(), (*asm_parsed)[findLabel(res.name,
      // asm_parsed)].line);
      asm_Error.error_message =
          string_format("label %s is already declare\n", res.getText());
    }
  ...
  if (sp.opcde.compare("bnez") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 2, op_bnez, 3, bin_bnez);
  ...
  if (sp.opcde.compare("beqz") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 2, op_bnez, 3, bin_beqz);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_bnez;
  ...
  if (sp.opcde.compare("bt") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 2, op_jumpfloat, 3, bin_bt);
    ps.op = opCodeType::jump;
  ...
  if (sp.opcde.compare("bf") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 2, op_jumpfloat, 3, bin_bf);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_blt;
  ...
  if (sp.opcde.compare("olt.s") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_floatco, 3, bin_olts);
    // ps.op = opCodeType::jump;
  ...
  if (sp.opcde.compare("oeq.s") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_floatco, 3, bin_oeqs);
    // ps.op = opCodeType::jump;
    // ps.calculateOfssetJump = jump_bge;
  ...
  if (sp.opcde.compare("blti") == 0) {
    ...
    result_parse_line ps =
        parseOperandes(sp.operandes, 3, op_blti, 3, bin_blti);
    ps.op = opCodeType::jump;
    ps.calculateOfssetJump = jump_blt;
  ...
  if (sp.opcde.compare("l32r") == 0) {
    result_parse_line ps =
        parseOperandes(sp.operandes, 2, op_l32r, 3, bin_l32r);
  ...
  if (sp.opcde.compare("call8") == 0) {
    result_parse_line ps =
        parseOperandes(sp.operandes, 1, op_call8, 3, bin_call8);
    ps.op = opCodeType::jump_32aligned;
  ...
  if (sp.opcde.compare("call12") == 0) {
    result_parse_line ps =
        parseOperandes(sp.operandes, 1, op_call8, 3, bin_call12);
    ps.op = opCodeType::jump_32aligned;
    ps.calculateOfssetJump = jump_call8;
    return ps;
  }
  ...
  if (sp.opcde.compare(".bytes") == 0) {
    ...
    if (depart.find_first_of(" ") < depart.size()) {
      ...
      value = strtol(depart.substr(0, depart.find_first_of(" ")).c_str(),
                     &endptr, 10);
    } else {
      suite = ""; // depart;
    ...
    //  int value = strtol(sp.operandes.substr(0, sp.operandes.find_first_of("
    //  ")).c_str(), &endptr, 10); printf("first of
    //  %d\n",sp.operandes.find_first_of(" ")); printf("on a
    //  /%s/%s/%d\n",depart.c_str(), suite.c_str(),value);

    if (*endptr == 0) {
      result_parse_line ps;
      ps.op = opCodeType::data;
      result_parse_line *ps1 = getInstrAtPos(
          (*asm_parsed).size() - 1); //&(*asm_parsed)[(*asm_parsed).size() - 1];
  ...
  if (sp.opcde.compare("callExt") == 0) {
    /*
    result_parse_line ps = parseOperandes(sp.operandes, 1, op_call8, 3,
    bin_call8); ps.op = opCodeType::external_call; ps.calculateOfssetJump =
    jump_call8; return ps;
    */
    ...
    ps = parseOperandes(string_format("a%d,%s", ps.bincode, ps.getText()), 2,
                        op_l32r, 3, bin_l32r);
    ps.op = opCodeType::jump_32aligned;
    ps.calculateOfssetJump = jump_l32r;
    ...
    ps = parseOperandes(string_format("a%d", savbin), 1, op_callx8, 3,
                        bin_callx8);
  ...
  if (sp.opcde.compare("movExt") == 0) {
    ...
    {
      asm_Error.error = 1;
      asm_Error.error_message = string_format("External variable %s not
    found\n", ps.getText());
    }
    else
    {
      // string debugsav=ps.debugtxt;
      ps = parseOperandes(string_format("a%d,a2,%d", ps.bincode, i * 4), 3,
    op_l32i, 3, bin_l32i);
      // ps.debugtxt=debugsav;
...
error_message_struct parseASM(Text *_footer, Text *_header, Text *_content,
                              parsedLines *asm_parsed) {
  ...
  _instr_size =
      (_nb_align_label + 1) * ALIGN_INSTR +
      (_content->size() + _footer->size() - _nb_not_aligned_label) * 3;
  ...
  string _d =
      string_format("Creation of an %d bytes binary and %d bytes data %d",
                    _instr_size, _size, (tmp_data_size / 4) * 4 + 4);
  pushToConsole(_d);
  ...
  string d =
      string_format("Parsing %d assembly lines ... ",
    ...
    if ((_content->_texts[0]) != NULL) {
      ...
      if (_content->front().compare(" ") != 0) {
        ...
        if (!res.error) {
          ...
          if (asm_Error.error) {
            main_error.error = 1;
            main_error.error_message += string_format(
                "line:%d  %s %s\r\n", i, _content->front().c_str(),
                asm_Error.error_message.c_str());
          } else {
            // printf("befoire line:%d mem:%u\r\n",i, esp_get_free_heap_size());
            // asm_parsed->push_back(re_sparse);
            addInstr(re_sparse, asm_parsed);
...
void printparsdAsm(uint32_t start_address, parsedLines *asm_parsed) {
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
    ...
    if (asm_Error.error) {
      // printf("%8x \t %s\t %s\n", re_sparse.address + start_address,
      // re_sparse.debugtxt.c_str(), re_sparse.error.error_message.c_str());
    } else {
      if (re_sparse.op == opCodeType::label) {
        printf("%8x \t <%s>:\n",
               (unsigned int)(re_sparse.address + start_address),
               re_sparse.getText());
      } else {
        // printf("%8x \t %6x\t %s\n", re_sparse.address + start_address,
        // re_sparse.bincode, re_sparse.debugtxt.c_str());
      }
    }
  }
}
...
void flagLabel32aligned(parsedLines *asm_parsed)
{
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin(); it !=
asm_parsed->end(); it++)
  {
    if ((*it)->op == opCodeType::jump_32aligned || (*it)->op ==
opCodeType::function_declaration)
    {
...
error_message_struct calculateJump(uint8_t *exec, parsedLines *asm_parsed) {
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
    ...
    if ((parse_line->op == opCodeType::jump) ||
        (parse_line->op == opCodeType::jump_32aligned)) {
      ...
      if (index != -1) {
        if (parse_line->calculateOfssetJump) {
          parse_line->bincode = parse_line->calculateOfssetJump(
              parse_line->bincode, parse_line->address,
              getInstrAtPos(index)->address);
          // printf("line:%d %d\r\n",parse_line->address,parse_line->size);
          memcpy(exec + parse_line->address, &parse_line->bincode,
                 parse_line->size);
        } else {
          error.error = 1;
          // error.error_message += string_format("asm_Error:No method to
          // calcuylate jump offset for %s\n", parse_line->debugtxt.c_str());
        }
      } else {
        error.error = 1;
        // printf("b ou foune\r\n");
        error.error_message +=
            string_format("line : %d label %s not found\n\r", parse_line->line,
                          parse_line->getText());
      }
    }
  }
...
uint8_t *createBinaryHeader(parsedLines *asm_parsed) {
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
       it != asm_parsed->end(); it++) {
    if ((*it)->op == opCodeType::data_label ||
        (*it)->op == opCodeType::number_label) {
      nb_objects++;
  ...
  for (vector<result_parse_line *>::iterator it = asm_parsed->begin();
    ...
    if ((*it)->op == opCodeType::data_label ||
      ...
      if ((*it2)->op == opCodeType::variable) {
        ...
        // printf(" qss %s %ld  %s\r\n", stackstring.c_str(),
        // getInstrAtPos(findLabel(stackstring, asm_parsed))->bincode,
        // (*it2)->getText());
...
Binary createBinary(Text *_footer, Text *_header, Text *_content,
                    bool display) {
  Binary bin;
