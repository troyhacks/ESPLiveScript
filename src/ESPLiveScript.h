...
void prettyPrint(NodeToken *_nd, string ident) {
  ...
  printf("%s%s\tisPointer:%d\tasPointer:%d\t", ident.c_str(),
         nodeTypeNames[nd._nodetype].c_str(), nd.isPointer,
         nd.asPointer); //, tokenNames[nd._token.type].c_str());

  printf("text:%s\ttokenType:%s\t", nd.getTokenText(),
         tokenNames[nd.type].c_str());
  // printf("\n");
  if (nd.getVarType() != NULL) {

    if (nd.type == (int)TokenUserDefinedVariable)
      printf("var name:%s\t total size:%d\tstackpos:%d\t",
             _userDefinedTypes[nd._vartype].varName.c_str(), nd._total_size,
             nd.stack_pos);
    else

      printf("var type:%s\ttotal size:%d\tstackpos:%d\t",
             varTypeEnumNames[nd._vartype].c_str(), nd._total_size,
             nd.stack_pos);
  }
...
public:
  ...
  string linepos() {
    string f = string_format(" at line:%d position:%d", current()->line,
                             current()->pos);
  ...
  Token *getTokenAtPos(int pos) { return _tks->getTokenAtPos(pos); }
  Token *current() { return _tks->current(); }
  Token *next() { return _tks->next(); }
  Token *prev() { return _tks->prev(); }
  Token *peek(int index) { return _tks->peek(index); }
  ...
  void getVariable(bool isStore) {
    ...
    if (search_result == NULL) {
      ...
      for (int i = 0; i < external_links.size(); i++) {
        if (strcmp(current()->getText(), external_links[i].name.c_str()) == 0) {
          ...
          extra_script.addContent(
              (char *)(external_links[i].signature.c_str()));
          extra_script.init();
...

      {
        Error.error = 1;
        Error.error_message =
            string_format("impossible to find declaraiton for %s %s",
                          current()->getText(), linepos().c_str());
        next();
        return;
      }
    }
    ...
    if (Match(TokenOpenBracket)) {
      ...
      if (Match(TokenCloseBracket)) {
        ...
        if (Match(TokenCloseBracket)) {
          ...
          if (nb < _s) {
            Error.error = 1;
            Error.error_message = string_format(
                "too many arguments expected less than %d got %d at %s", nb, _s,
                linepos().c_str());
            next();
            return;
          }
          ...
          Error.error = 1;
          Error.error_message =
              string_format("expecting ]  or , %s at %s", current()->getText(),
                            linepos().c_str());
          next();
          return;
        }
    ...
    if (Match(TokenMember) && Match(TokenIdentifier, 1) &&
        !Match(TokenOpenParenthesis, 2)) {
      next();
      int i = 0;
      varType *v = NULL;

      if (current_node->_vartype == __CRGB__ or
          current_node->_vartype == __CRGBW__) {
        ...
        if (i < 0) {
          Error.error = 1;
          Error.error_message =
              string_format("Member %s of struct %s does not exists",
                            current()->getText(), v->varName.c_str());
          next();
          return;
        }
      } else {
      ...
      if (current_node->isPointer) {
        // current_node->_total_size =1000* current_node->_total_size
        // +v->sizes[i];
      } else {
        current_node->_total_size = v->sizes[i];
      }
      next();
    } else if (Match(TokenMember, 0) && Match(TokenIdentifier, 1) &&
               Match(TokenOpenParenthesis, 2)) {
      ...
      current()->addText(
          string_format("%s.%s", search_result->getVarType()->varName.c_str(),
                        current()->getText()));
      ...
      nd.isPointer = true;
      nd._total_size = search_result->getVarType()
                           ->total_size; // nd.copyChildren(current_node);
                                         /*
      ...
      current_node->getChildAtPos(current_node->children_size() - 1)
          ->getChildAtPos(0)
          ->getChildAtPos(0)
          ->copyChildren(_node_token_stack.back());
      _node_token_stack.pop_back();
  ...
  void parseArguments() {
    ...
    while (Match(TokenComma)) {
      ...
      string _signature =
          sigs.back() + "|" + current_node->getVarType()->varName;
      sigs.pop_back();
      sigs.push_back(_signature);
      if (current_node->isPointer) {
  ...
  void parseFunctionCall() {
    ...
    if (!main_context.findCandidate(sav_t.back().getText())) {
      if (struct_name.size() > 0) {
        string v_tmp =
            string_format("%s.%s", struct_name.c_str(), sav_t.back().getText());
    ...
    (&sav_t.back())
        ->addText(
            string_format("%s%s", sav_t.back().getText(), sigs.back().c_str()));
    sigs.pop_back();
    ...
    if (search_result == NULL) {
      ...
      if (struct_name.size() > 0) {
        ...
        (&sav_t.back())
            ->addText(string_format("%s.%s", struct_name.c_str(),
                                    sav_t.back().getText()));
        main_context.findFunction(&sav_t.back());
        isStructFunction = true;
        sav_t.pop_back();
      ...
      if (search_result == NULL) {
        ...
        for (int i = 0; i < external_links.size(); i++) {
          ...
          if (strstr(external_links[i].signature.c_str(), "Args") != NULL) {
            int l = strstr(external_links[i].signature.c_str(), "Args") -
                    external_links[i].signature.c_str();
            if (l > 0)
              l--;
            if (strncmp(external_links[i].signature.c_str(),
                        sav_t.back().getText(), l) == 0) {
              found = true;
            }
          ...
          if (found) {
            ...
            _node_token_stack.push_back(current_node);
            current_node = &program;
            // string toinsert = external_links[i].name; //"external " +
            // external_links[i].out + " " + external_links[i].name +
            // "("+external_links[i].in + ");"; printf("on inseet %s\n",
        ...
        if (search_result == NULL) {
          Error.error = 1;
          Error.error_message =
              string_format("function %s not found %s", sav_t.back().getText(),
                            linepos().c_str());
          return;
        }
    ...
    // if (search_result->getChildAtPos(0)->_vartype == __float__ and
    // change_type.size() > 0)
    //   change_type.back()->_vartype = __float__;

    if (change_type.size() > 0) {
      if (search_result->getChildAtPos(0)->children_size() == 0 &&
          !search_result->getChildAtPos(0)->asPointer)
        change_type.back()->isPointer =
            search_result->getChildAtPos(0)->isPointer; // n,ew modif here
      if (change_type.back()->_vartype != __float__) {
        if (search_result->getChildAtPos(0)->_vartype == __float__ ||
            search_result->getChildAtPos(0)->_vartype == __uint32_t__) {
          change_type.back()->_vartype =
              search_result->getChildAtPos(0)->_vartype;
        }
      }
    }
    //  NodeToken *o =
    //  current_node->addChild(NodeToken(search_result->getChildAtPos(1)));
    ...
    if (nb_sav_args.back() != nb_args.back() and
        nb_sav_args.back() != 999) // if (sav_nb_arg != nb_args.back())
    {
      Error.error = 1;

      Error.error_message =
          string_format("Expected %d arguments got %d %s", nb_sav_args.back(),
                        nb_args.back(), linepos().c_str());
      return;
    }
    nb_args.pop_back();
    nb_sav_args.pop_back();
  ...
  void parseStatement() {
    ...
    if (Match(TokenString)) {
      ...
      if (c.compare("__") == 0) {
        Error.error = 1;
        Error.error_message = string_format(
            "nor For ar while found for break %s", linepos().c_str());
        return;
      }
      next();
      if (Match(TokenSemicolon)) {
        ...
        Error.error = 1;
        Error.error_message =
            string_format("d Expected ; %s", linepos().c_str());
        return;
      }
      ...
      if (c.compare("__") == 0) {
        Error.error = 1;
        Error.error_message = string_format(
            "nor For ar while found for continue %s", linepos().c_str());
        return;
      }
      next();
      if (Match(TokenSemicolon)) {
      ...
      if (Match(TokenSemicolon)) {
        ...
        if (lasttype != NULL) {
          ...
          */
          ...
          Error.error = 1;
          Error.error_message =
              string_format("issue with return %s", linepos().c_str());
          return;
        }
      ...
      if (Error.error) {
        ...
        if (Match(TokenSemicolon)) {
          ...
          Error.error = 1;
          Error.error_message =
              string_format("Expected ; %s", linepos().c_str());
          next();
          return;
        }
      }
    ...
    if (Match(TokenIdentifier) &&
        (Match(TokenPlusPlus, 1) or Match(TokenMinusMinus, 1))) {
      ...
      if (Match(TokenEqual)) {
        ...
        */
        ...
        if (!Match(TokenSemicolon) && !Match(TokenCloseParenthesis)) {
          Error.error = 1;
          Error.error_message =
              string_format("Expected ici ; %s", linepos().c_str());
          // next();
          return;
        }
        ...
        next();
        return;
      } else if (Match(TokenPlusEqual) || Match(TokenMinusEqual) ||
                 Match(TokenStarEqual) || Match(TokenSlashEqual)) {
        sav_t.push_back(*current());
        // NodeToken nd;
        NodeToken nd = NodeToken(changeTypeNode);
        nd._nodetype = changeTypeNode;
      ...
      // current_cntx->_global->children_size(),
      // current_cntx->_global->name.c_str(), current()->text.c_str(),
      // (uint64_t)current_cntx->_global); current_cntx =
      // (*(current_cntx)).addChild(cntx);
      ...
      current_node = current_node->addChild(
          NodeToken(current(), elseNode, targetList.pop()));
      next();
      ...
      if (Match(TokenOpenParenthesis)) {
        ...
        // current_node = current_node->addChild(ndf);
        current_node = current_node->addChild(
            NodeToken(&sav_t.back(), whileNode, targetList.get()));
        next();
        ...
        Error.error = 1;
        Error.error_message =
            string_format("expecting ( %s", linepos().c_str());
        next();
        return;
      }
      ...
      // current_cntx->_global->children_size(),
      // current_cntx->_global->name.c_str(), current()->text.c_str(),
      // (uint64_t)current_cntx->_global); Context *k =
      // (*(current_cntx)).addChild(cntx);
      current_cntx = current_cntx->addChild(Context());
      // string target =string_format("label_%d%s",for_if_num,k->name.c_str());
      targetList.push(string_format("label_%d", for_if_num));
      ...
      if (Match(TokenOpenParenthesis, 1)) {
        ...
        // current_node = current_node->addChild(ndf);
        current_node = current_node->addChild(
            NodeToken(current(), ifNode, targetList.get()));
        next();
        next();
      ...
      // current_cntx = k;
      // string target
      // =string_format("label_%d%s",for_if_num,current_cntx->name.c_str());
      targetList.push(string_format("label_%d", for_if_num));
      //=target;
      for_if_num++;
      // next();
      if (Match(TokenOpenParenthesis, 1)) {
        ...
        // current_node = current_node->addChild(ndf);
        current_node = current_node->addChild(
            NodeToken(current(), forNode, targetList.get()));
        next();
      ...
      nodeTokenList.push(createNodeLocalVariableForCreation(
          nodeTokenList.pop(), nodeTokenList.pop()));
      ...
      if (Match(TokenSemicolon)) {
        ...
        if (nodeTokenList.get().type == TokenUserDefinedVariable) {
          ...
          current()->addText(string_format(
              "%s._@%s()", nodeTokenList.get().getVarType()->varName.c_str(),
              nodeTokenList.get().getVarType()->varName.c_str()));
      ...
      if (Match(TokenEqual)) {
        ...
        if (nodeTokenList.get().type == TokenUserDefinedVariable) {
          ...
          next();
          current()->addText(string_format(
              "%s._@%s", search_result->getVarType()->varName.c_str(),
              current()->getText()));
          NodeToken nd = NodeToken(*search_result);
          ...
          if (!Match(TokenSemicolon)) {
            Error.error = 1;
            Error.error_message =
                string_format("Missing ; %s", linepos().c_str());
            return;
          }
          next();
          //       current_node->getChildAtPos(current_node->children_size() -
          //       1)->getChildAtPos(2)->getChildAtPos(0)->copyChildren(par);
          isStructFunction = false;
          Error.error = 0;
          // current_node = current_node->parent;
          return;
        ...
        } else if (_uniquesave.getNodeTokenType() ==
        ...
        current_node->addChild(
            _uniquesave); // createNodeLocalVariableForStore(nodeTokenList.pop()));
                          // _uniquesave->text=current_cntx->variables.back().text;
                          // NodeToken nd;
        NodeToken nd = NodeToken(changeTypeNode);
        nd._nodetype = changeTypeNode;
        ...
        Error.error = 1;
        Error.error_message =
            string_format(" expecting ;  ou = %s", linepos().c_str());
        return;
      }
  ...
  void parseDefFunction() {
    ...
    if (search_result !=
        NULL) // if (current_cntx->findFunction(current()) != NULL)
    {
      ...
      if (search_result->_nodetype != declarationFunctionNode) {
        ...
        Error.error = 1;
        Error.error_message =
            string_format("function already %s  declared in the scope for %s",
                          current()->getText(), linepos().c_str());
        next();
        return;
      } else {
    ...
    // signature.c_str());
    current_node->setTokenText(
        string_format("%s%s", current_node->getTokenText(), signature.c_str()));
    if (!isdeclaration) {
    ...
    if (ext_function) {
      if (Match(TokenSemicolon)) {
        ...
        Error.error = 1;
        Error.error_message = string_format(
            "Impossible to redifine external function expected ; %s",
            linepos().c_str());
        next();
        return;
      }
  ...
  void parseTerm() {
    ...
    while (Match(TokenStar) || Match(TokenSlash) || Match(TokenModulo) ||
           Match(TokenKeywordOr) || Match(TokenKeywordAnd) ||
           Match(TokenPower)) {
      // token *op = current();
      sav_t.push_back(*current());
      next();
  ...
  void parseExprConditionnal() {
    ...
    while (Match(TokenDoubleEqual) || Match(TokenLessOrEqualThan) ||
           Match(TokenLessThan) || Match(TokenMoreOrEqualThan) ||
           Match(TokenMoreThan)) {
  ...
  void parseExpr() {
    ...
    while (Match(TokenAddition) || Match(TokenSubstraction) ||
           Match(TokenShiftLeft) || Match(TokenShiftRight)) {
  ...
  void parseFactor() {
    ...
    if (current()->getType() == TokenEndOfFile) {
    ...
    else if (Match(TokenNot) || Match(TokenAddition) ||
             Match(TokenSubstraction) || Match(TokenUppersand) ||
             Match(TokenKeywordFabs) || Match(TokenKeywordAbs)) {
      // token *t = current();
      // NodeUnitary g = NodeUnitary();
      current_node = current_node->addChild(NodeToken(unitaryOpNode));
      sav_t.push_back(*current());
    ...
    else if (Match(TokenOpenParenthesis) && Match(TokenKeywordVarType, 1) &&
             Match(TokenCloseParenthesis, 2) &&
             Match(TokenOpenParenthesis, 3)) {
      // on a (float) ....;
      next();
      ...
      if (Match(TokenCloseParenthesis)) {
        ...
        Error.error = 1;
        Error.error_message =
            string_format("expected ) at %s", linepos().c_str());
        return;
      }
    ...
    else if (Match(TokenUserDefinedVariable) &&
             Match(TokenOpenParenthesis, 1)) {
      sav_b = isStructFunction;
      isStructFunction = true;
      ...
      current()->addText(
          string_format("%s._@%s", current()->getText(), current()->getText()));
      parseFunctionCall();
      if (Error.error) {
        return;
      }
        ...
        for (int i = 0; i < sav_t.back().getVarType()->size; i++) {
          ...
          if (i == sav_t.back().getVarType()->size - 1) {
            if (!Match(TokenCloseParenthesis)) {
              ...
              Error.error = 1;
              Error.error_message =
                  string_format(" ecpected  ) %s", linepos().c_str());
              next();
              return;
            }
          } else {
      ...
      nd.isPointer = true;
      nd.textref =
          all_text.addText(string_format("local_string_%d", for_if_num));
      for_if_num++;
      ...
      // NodeToken *f = program.addChildFront(nd)->addChild(NodeToken(current(),
      // stringNode));
  ...
  void parseType() {
    ...
    if (Match(TokenKeywordVarType) or Match(TokenUserDefinedVariable)) {
      ...
      Error.error = 1;
      Error.error_message = string_format(
          "expecting external, __ASM__  or variable type %s %d %s",
          linepos().c_str(), current()->type, current()->getText());
      next();
      return;
    }
    Error.error = 0;
  ...
  void parseVariableForCreation() {
    ...
    if (search_result != NULL) {
      ...
      Error.error = 1;
      Error.error_message =
          string_format("variable %s already declared in the scope for %s",
                        current()->getText(), linepos().c_str());
      next();
      return;
    }
    if (Match(TokenOpenBracket, 1)) {
      ...
      if (Match(TokenNumber)) {
        ...
        if (current()->getVarType()->_varType == __int__) {
          ...
          Error.error = 1;
          Error.error_message =
              string_format("expecting an integer %s", linepos().c_str());
          next();
          return;
        }
        while (Match(TokenComma)) {
        ...
        if (Match(TokenCloseBracket)) {
          var.isPointer = true;
          var._nodetype = defGlobalVariableNode; // we can't have arrays in the
                                                 // stack var._total_size =
                                                 // stringToInt(num.getText());
        ...
        var.isPointer = true;
        var._nodetype =
            defGlobalVariableNode; // we can't have arrays in the stack
        ...
        Error.error = 1;
        Error.error_message =
            string_format("expecting an integer %s or ]", linepos().c_str());
        next();
        return;
      }
    } else {
  ...
  void parseProgram() {
    ...
    while (Match(TokenEndOfFile) == false) {
      ...
      if (Match(TokenUnknown)) {
        Error.error = 1;
        Error.error_message =
            string_format("Unkown Token %s", linepos().c_str());
        return;
      }
      if (Match(TokenKeywordStruct)) {
        ...
        if (Match(TokenUserDefinedName)) {
          ...
          while (!Match(TokenCloseCurlyBracket) and !Match(TokenEndOfFile)) {
            ...
            if (Match(TokenUserDefinedVariable) and
                Match(TokenOpenParenthesis, 1)) {
              isStructFunction = true;
              NodeToken _nd = NodeToken(UnknownNode);
              _nd._nodetype = typeNode;
              ...
              isStructFunction = false;
            } else if (Match(TokenKeywordVarType) and
                       Match(TokenIdentifier, 1) and
                       Match(TokenOpenParenthesis, 2)) {
              isStructFunction = true;
              parseType();
              if (Error.error) {
              ...
              if (!Match(TokenSemicolon)) {
                Error.error = 1;
                Error.error_message =
                    string_format("missing ; at line %s", linepos().c_str());
                return;
              }
              next();
              memberpos++;
              ...
              Error.error = 1;
              Error.error_message =
                  string_format("Invalid statement line %s", linepos().c_str());
              return;
            }
          }
          usded.size = _pos;
        ...
        if (Match(TokenIdentifier)) {
          if (Match(TokenOpenParenthesis, 1)) {
            ...
            if (Match(TokenComma)) {
              ...
              if (current_node->type == TokenUserDefinedVariable) {
                current()->addText(string_format(
                    "%s._@%s()", current_node->getVarType()->varName.c_str(),
                    current_node->getVarType()->varName.c_str()));
                main_context.findFunction(current());
            ...
            else if (Match(TokenEqual) and
                     Match(TokenUserDefinedVariable, 1) and
                     Match(TokenOpenParenthesis, 2) and
                     current_node->type == TokenUserDefinedVariable) {
          ...
          Error.error = 1;
          Error.error_message =
              string_format("expected identifier %s", linepos().c_str());
          next();
          return;
        }
      }
...
void compile_c(Console *cons, vector<string> args) {
  ...
  if (_scExec.exeExist) {
    ...
    pushToConsole(string_format("Compiling done. Handle number:%d",
...
void saveBin(Console *cons, vector<string> args) {
  ...
  if (args.size() > 0) {
    ...
    saveBinary((char *)(fileSystem.current_path + args[0]).c_str(),
               *fileSystem.current_mount->fs, &bin);
    freeBinary(&bin);
  } else
...
void displaybin(Console *cons, vector<string> args) {
  Binary bin;
  if (args.size() > 0) {
    loadBinary((char *)(fileSystem.current_path + args[0]).c_str(),
               *fileSystem.current_mount->fs, &bin);
    displayBinary(&bin);
    freeBinary(&bin);
  }
...
public:
  __INIT_PARSER() {
    ...
    LedOS.addKeywordCommand(
        "free", free,
        "free the binary free x will free the program with handle x");
    LedOS.addKeywordCommand("run", run,
                            "Run an already compiled program (always second "
                            "Core) run x run program with handle x");
    LedOS.addKeywordCommand(
        "kill", kill,
        "Stop a running program kill x kill program with handle x");
    LedOS.addKeywordCommand("parseasm", parseasm, "Parse assembly program");
    LedOS.addEscCommand(18, parsec_cEsc,
                        "Compile and execute a program (always second Core)");
...
string formatLine(string str) {
  ...
  while (_tks->current()->getType() !=
         TokenEndOfFile) // for (int i = 0; i < _tks->size(); i++)
  {
    ...
    /* if (tk.type == TokenOpenCurlyBracket)
       ...
       res = res + string_format("%s%s", _colors[_curlybracket.size() %
       _NB_COLORS], tk.text.c_str());
    ...
    if (tk.getType() == TokenOpenParenthesis) {
      _prevparenthesis++;
      res = res + string_format("%s%s",
                                _colors[(_parenthesis.size() + 2) % _NB_COLORS],
                                tk.getText());
      ...
      if (_parenthesis.size() == 0) {
        res = res + string_format("%s%s", "\u001b[38;5;196m", tk.getText());
      } else {
        _prevparenthesis--;
        res = res + string_format("%s%s", _parenthesis.back(), tk.getText());
        _parenthesis.pop_back();
      }
    } else if (tk.getType() == TokenOpenBracket) {
      _prevbracket++;
      res = res + string_format("%s%s", _colors[_bracket.size() % _NB_COLORS],
                                tk.getText());
      ...
      if (_bracket.size() == 0) {
        ...
        _prevbracket--;
        res = res + string_format("%s%s", _bracket.back(), tk.getText());
        _bracket.pop_back();
      }
    }
    /*
       ...
       {
       res = res + string_format("%s%s", KeywordTypeFormat[tk._keyword],
       tk.text.c_str());
       } */
    else {
      Token tkn = *_tks->peek(1);
      if (tk.getType() == TokenIdentifier &&
          tkn.getType() == TokenOpenParenthesis) {
        res = res +
              string_format("%s%s", tokenFormat[TokenFunction], tk.getText());
      } else {

        res = res + string_format("%s%s", tokenFormat[tk.type], tk.getText());
      }
      // _tks->prev();
    }
    _tks->next();
