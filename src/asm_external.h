...
void addExternalFunction(string name, string out, string in, void *ptr) {
  ...
  if (out.size() == 0) {
    ...
    for (int i = 0; i < j.size(); i++) {
      ...
      asmex.name =
          string_format("%s%s a%d", asmex.name.c_str(), j[i].c_str(), i);
      if (i < j.size() - 1) {
        asmex.signature = asmex.signature + "|";
...
void replaceExternal(string name, void *ptr) {
  ...
  for (vector<asm_external>::iterator it = external_links.begin();
...
int findLink(string label, externalType op) {
  ...
  for (int i = 0; i < external_links.size(); i++) {
    if ((external_links[i].shortname.compare(label) ==
         0)) //&&  (external_links[i].type==op))
    {
      return i;
    }
  }
