...
public:
  ...
  void setFunctionToSync(void (*function)()) { functionToSync = function; }
...
public:
  ...
  int _run(vector<string> args, bool second_core, int core,
    ...
    if (exeExist == true) {
      ...
      xTaskCreateUniversal(
          _run_task, taskname.c_str(), 4096 * 2, this, 3,
          (TaskHandle_t *)runningPrograms.getHandleByIndex(__run_handle_index),
          core);
  ...
  void executeAsTask(string prog, int core, Arguments arguments) {
    ...
    if (core == 0 or core == 1) {
      ...
      pushToConsole(
          string_format("Wrong core number %d should be 0 or 1", core));
    }
#endif
  }
...
static void _run_task(void *pvParameters) {
  ...
  if (exec->df.args.size() > 0) {
    error_message_struct res = executeBinary("@__footer", exec->df.exe,
                                             exec->__run_handle_index, exec, d);

    res = executeBinary(exec->df.args[0], exec->df.exe,
                        exec->__run_handle_index, exec, exec->args);
    if (res.error) {
      pushToConsole(res.error_message, true);
    }
  } else {
...
public:
  ...
  void deleteExe(string name) {
    ...
    if (exec != NULL) {
      kill(name);
      free(name);
      for (vector<Executable>::iterator it = _scExecutables.begin();
  ...
  void listExec() {
    for (int i = 0; i < _scExecutables.size(); i++) {
      ...
      pushToConsole(
          string_format(" %2d | %20s isRunning:%d| %6d | %6d |%s", i + 1,
                        _scExecutables[i].name.c_str(),
                        _scExecutables[i].isRunning(),
                        _scExecutables[i]._executecmd.binary_size,
                        _scExecutables[i]._executecmd.data_size,
                        _scExecutables[i].error.error_message.c_str()),
          true);
    }
  }
